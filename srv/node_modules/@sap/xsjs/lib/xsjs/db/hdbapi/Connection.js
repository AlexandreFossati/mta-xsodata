...
var executeBatch = require('../common/execute-batch');
...
Connection.prototype.executeQuery = function () {
  ...
  var rs = execute.sync.call(null, this._tracer, this._client, args.shift(), args);
...
Connection.prototype.executeUpdate = function () {
  var args = Array.prototype.slice.call(arguments);
  args = normalizeInput(args, this._treatDateAsUTC);
  return execute.sync.call(null, this._tracer, this._client, args.shift(), args);
};
...
function execute(tracer, client, sql, sqlArgs, callback) {
  if (!_.isString(sql)) {
    return callback(new Error('Invalid sql'));
  }
  ...
  client.prepare(sql, function (err, statement) {
    ...
    try {
      ...
      return afterStatementDrop(tracer, statement, function () {
        callback(err);
      });
    }
    ...
    if (fnCode === FunctionCode.NIL || fnCode === FunctionCode.DDL ||
      ...
      async.waterfall([
        function (cb) {
          if (isBatch) {
            executeBatch(statement, args, cb);
          } else {
            statement.execute(args, cb);
          }
        }
      ], function () {
        var results = arguments;
        afterStatementDrop(tracer, statement, function () {
      ...
      statement.execute(args, function (err, rows) {
        if (err) {
          return afterStatementDrop(tracer, statement, function () {
            callback(err);
          });
        ...
        // rows will not be present if a procedure without output tables is called.
        // Executing CALL statements for procedures is not supported in XSC, but works in xsjs.
        rows = rows || [];
...
function afterStatementDrop(tracer, stmt, fn) {
