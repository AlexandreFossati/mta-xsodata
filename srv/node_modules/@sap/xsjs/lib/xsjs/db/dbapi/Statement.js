...
Statement.prototype.execute = function () {
  ...
  function executeStatement(cb) {
    ...
    case FunctionCode.SELECT_FOR_UPDATE:
      self._statement.execute(self._parameters, { rowsAsArray: true }, function (err) {
        if (err) {
          return cb(err);
        }

        if (functionCode === FunctionCode.INSERT || functionCode === FunctionCode.UPDATE || functionCode === FunctionCode.DELETE) {
          return cb(null, { rowsAffected: arguments[1] });
        }

        if (functionCode === FunctionCode.SELECT || functionCode === FunctionCode.SELECT_FOR_UPDATE) {
          return cb(null, { setsOfRows: [ { rows: arguments[1], columnInfo: self._statement.getColumnInfo() } ] });
        }
    ...
    case FunctionCode.DB_PROCEDURE_CALL_WITH_RESULT:
      self._statement.executeQuery(self._parameters, function (err, resultSet) {
        if (err) {
          return cb(err);
        }

        var result = {};
        result.parameters = transformProcParameters(self._statement);
        ...
        var setsOfRows = [];
        fetchAllResults(setsOfRows, resultSet, function (err) {
          resultSet.close(function () { // ignoring error here
            if (err) {
              return cb(err);
            }
    ...
    default:
      cb(new Error('Invalid or unsupported FunctionCode ' + functionCode));
    }
  }
...
function fetchAllResults(collection, resultSet, cb) {
  fetchRows(resultSet, 'array', function (err, rows) {
    if (err) {
      return cb(err);
    }
    collection.push({ rows: rows, columnInfo: resultSet.getColumnInfo() });

    resultSet.nextResult(function (err, hasNextResultSet) {
      if (err) {
        return cb(err);
      }
      ...
      fetchAllResults(collection, resultSet, cb);
    });
  });
}
