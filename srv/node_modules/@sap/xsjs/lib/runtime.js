...
var node10AndUp = parseInt(process.versions.node) >= 10;
...
Runtime.prototype.importLibrary = function (packagename, objectname, xsruntime) {
  ...
  loadLibrary(sandbox, script, library);
...
function loadLibrary(sandbox, script, library) {
  var keys = Object.getOwnPropertyNames(sandbox);
  script.runInNewContext(sandbox);
  // add new properties to library
  getLibProps(sandbox).filter(function (key) {
...
function getLibProps(sandbox) {
  if (node10AndUp) {
    // Workaround for https://github.com/nodejs/node/issues/27783
    const libObj = vm.runInContext('this', sandbox);
    return Object.keys(libObj);
  }
...
Runtime.prototype.attachContext = function (req, res, traceOptions) {
  ...
  var webResponse = new xs.web.WebResponse(res, { runtime: this, context: baseContext, loggingContext: req.loggingContext });
...
function xsjs(pathname, filename) {
  ...
  this.xsjs[pathname] = readScript.call(this, filename);
}
...
function xsjslib(pathname, filename) {
  ...
  this.xsjslib[utils.toResourceId(pathname)] = readScript.call(this, filename);
}
